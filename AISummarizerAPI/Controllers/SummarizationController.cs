...
public class SummarizationController : ControllerBase
{
    ...
    {
        _logger.LogInformation("Received summarization request for content type: {ContentType}", request.ContentType);

        try
        {
            if (!ModelState.IsValid)
            {
                _logger.LogWarning("Invalid model state for summarization request");
            ...
            switch (request.ContentType.ToLowerInvariant())
            {
                ...
                default:
                    ...
                    return BadRequest(new { Error = "Unsupported content type. Use 'text' or 'url'." });
            }
    ...
    public ActionResult GetSupportedContentTypes()
    {
        ...
        // Fix: Use explicit object creation instead of anonymous arrays
